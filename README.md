...
6. Copy/paste the entire snippet to a `.js` file in your project (you can find the example in this repo under `/newrelic/newrelic-browser-agent.js` - see [here](newrelic/newrelic-browser-agent.js))
...
8. Create a new React Component and import the New Relic Browser agent to the file
   1. see the example [here](components/NewRelicSnippet.tsx) - you can simply copy/paste it (remember to change the path to your New Relic Browser agent script if it differs)
9. Add the newly created component (in this case `NewRelicSnippet`) to the `Head` of your app entry point, usually `_app.tsx` under `pages` directory
   1. see how to do it [here](pages/_app.tsx#L12)
...
7. Add New Relic to your application's main module (NodeJs app entry point - usually `index.ts/js` - example [here](server/index.ts#L1))
